...
public class Day7 {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for(ArrayList<Long> list : numbersToOperate){
            ...
            fillAllCombsOfOperands(operators, "", 2, list.size()-1, combinationsOfOperators);
            for(String oneComb : combinationsOfOperators){
                long tempSum = 0;
                char[] oneCombArray = oneComb.toCharArray();
                char ch = oneCombArray[0];
                for(int i = 0; i < list.size(); i++){
                    ...
                    if(i != oneCombArray.length && !(i > oneCombArray.length) && i != 0){
                        ch = oneCombArray[i];
                    }
                }
                System.out.println("Temp sum: " + tempSum);
                System.out.println("Results: " + results.get(k));
