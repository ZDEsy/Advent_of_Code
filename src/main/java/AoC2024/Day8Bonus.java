        ...
        for(int i = 0; i < chars.size(); i++){
            for(int j = 0; j < chars.size(); j++){
                if(i != j && chars.get(i) == chars.get(j) && chars.get(i) != '.' && chars.get(j) != '.'){
                    correctPositions.add(new int[]{positions.get(j)[0], positions.get(j)[1]});
                    ...
                    while (newPosition[0] >= 0 && newPosition[1] >= 0 && newPosition[0] < board.size() && newPosition[1] < board.get(newPosition[0]).size()){
                        if(board.get(newPosition[0]).get(newPosition[1]) == '.'){
                            ...
                            newPosition = new int[]{newPosition[0] + distance[0], newPosition[1] + distance[1]};
                        }
